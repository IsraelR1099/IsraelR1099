...
/*   Updated: 2022/12/05 13:45:18 by irifarac         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*ft_action(void *arg)
{
	...
	printf("philo id %d, right fork %p\n", philo->id, &philo[philo->id].left_fork);
...
void	ft_start(t_philo *philo, int nbr)
{
	...
	while (i < nbr)
	{
		printf("start id %d, address left %p\n", philo[i].id, &philo[i].left_fork);
		printf("start id %d, address right %p\n", philo[i].id, &philo[i + 1].left_fork);
		i++;
	}
	i = 0;
	while (i < nbr)
