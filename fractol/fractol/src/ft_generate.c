...
/*   Updated: 2022/05/06 14:05:16 by irifarac         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_Julia(window *init)
{
	...
	while (init->values->b_temp >= -3)
	{
		...
		{
			init->values->new_a = ((4 * HEIGHT / 8) + init->trl) + ((init->zoom_x * init->values->a_temp * HEIGHT / 8)); 
			init->values->new_b = ((3 * WIDTH / 6)) - (init->zoom_y * init->values->b_temp * WIDTH / 6);
