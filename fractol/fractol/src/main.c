...
/*   Updated: 2022/05/23 14:15:17 by irifarac         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int counter, char **str)
{
	...
	init->inf = (points *)malloc(sizeof(points));
	if (!init || !init->inf)
	{
		free(init->inf);
		free(init);
	...
	if (counter > 1 && counter < 6)
	{
		...
		else if (counter == 5)
		{
			...
			init->inf->y_julia = ft_atoi(str[4]);
			generate_image(init, &*(*(str + 1) + 0));
		}
...
void	my_mlx_pxput(window *init, int x, int y, int color)
{
	char	*dst;
