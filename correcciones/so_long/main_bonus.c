...
void	count_another_move(t_mlx *mlx)
{
	if (mlx->hero->alive)
	{
		...
		mlx_put_image_to_window(mlx->mlx_ptr, mlx->win,
			mlx->img[5].img_ptr, 100, (mlx->map_height * 26));
		place_text_moves_to_window(mlx, ft_itoa(mlx->num_moves));
	}
}
...
int	render(t_mlx *mlx)
{
	...
	{
		check_enemy_collition(mlx, count);
		count++;
	}
	return (1);
}
