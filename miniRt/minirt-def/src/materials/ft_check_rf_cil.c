...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/05/31 09:46:22 by irifarac          #+#    #+#             */
/*   Updated: 2023/05/31 13:40:37 by irifarac         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static double	ft_calc_t(t_cylinder *cyl, t_ray *ray, double scalar[3])
{
	...
	if (ret_positive == ret_negative)
		return (0);
	if (ret_negative > KEPSILONSP && ft_cyl_inter(cyl, ray, limits, ret_negative))
		return (ret_negative);
	else if (ret_positive > KEPSILONSP && ft_cyl_inter(cyl, ray, limits, ret_positive))
		return (ret_positive);
	return (0);
}

double	ft_check_rf_cil(t_cylinder *cyl, t_ray *ray, bool *plane)
{
	double	scalar[3];
	double	ret;
